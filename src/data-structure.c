...
int minX(struct QTree *tree) {
    ...
    if (tree->leftover == NULL && tree->leftunder == NULL) {
        return MIN(minX(tree->rightover), minX(tree->rightunder));
    } else {
        return MIN(minX(tree->leftover), minX(tree->leftunder));
    }
}
...
int maxX(struct QTree *tree) {
    ...
    if (tree->rightover == NULL && tree->rightunder == NULL) {
        return MAX(maxX(tree->leftover), maxX(tree->leftunder));
    } else {
        return MAX(maxX(tree->rightover), maxX(tree->rightunder));
    }
}
...
int minY(struct QTree *tree) {
    ...
    if (tree->leftover == NULL && tree->rightover == NULL) {
        return MIN(minY(tree->leftunder), minY(tree->rightunder));
    } else {
        return MIN(minY(tree->leftover), minY(tree->rightover));
    }
}
...
int maxY(struct QTree *tree) {
    ...
    if (tree->leftunder == NULL && tree->rightunder == NULL) {
        return MAX(maxY(tree->leftover), maxY(tree->rightover));
    } else {
        return MAX(maxY(tree->leftunder), maxY(tree->rightunder));
    }
}
...
bool treeFits(struct QTree *tree, int x, int y) {
    ...
    bool x_in_range = (x <= tree->x && tree->x - x < tree->n) || (x > tree->x && x - tree->x <= tree->n);
    bool y_in_range = (y <= tree->y && tree->y - y < tree->n) || (y > tree->y && y - tree->y <= tree->n);
...
// If parent == NULL, sets the tree itself as its own parent
struct QTree *newTree(int x, int y, int n, struct QTree *parent) {
    struct QTree *tree = malloc(sizeof(struct QTree));

    *tree = (struct QTree){.x = x, .y = y, .n = n, .leftunder = NULL, .leftover = NULL, .rightunder = NULL, .rightover = NULL, .parent = parent};
    return tree;
}
...
struct QTree *cleanBranch(struct QTree *tree) {
    ...
    if (!tree->leftunder && !tree->leftover && !tree->rightunder && !tree->rightover) {
        struct QTree *parent = tree->parent;
        freeTree(tree);
...
struct QTree *insertCell(struct QTree *tree, int x, int y) {
    ...
    while (true) {
        ...
        if (next == NULL) {
            ...
            if (x <= tree->x) {
                if (y <= tree->y) {
                    containingNode = newTree(tree->x - (tree->n / 2), tree->y - (tree->n / 2), tree->n / 2, tree);
...
void extendTreeToFit(struct QTree *tree, int x, int y) {
    // TODO how to handle trees with n == 0?
    // My idea right now would be to just set n=2^m for m ∈ ℕ_0 and m > max(x,y) but I'm not sure if that's sensible
    while (!treeFits(tree, x, y)) {
        // NOTE: lift is just a copy of the original tree node and then the original tree node is extended and gets lift as only child
        struct QTree *lift = malloc(sizeof(struct QTree));
        *lift = (struct QTree){.x = tree->x,
                               .y = tree->y,
                               .n = tree->n,
                               .leftover = tree->leftover,
                               .leftunder = tree->leftunder,
                               .rightover = tree->rightover,
                               .rightunder = tree->rightunder,
                               .parent = tree};
